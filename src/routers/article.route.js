...
router.post("/addArticle", auth, multer({ storage: storage }).single("image"), async(req, res) => {
    const file = req.file;
    try {
...
router.post("/editArticle", auth, multer({ storage: storage }).single("image"), async(req,res)=>{
    const file = req.file;
    try {
        const data = req.body;
